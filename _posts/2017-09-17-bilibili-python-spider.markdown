...
categories: network learn python
---
...
可见整个网页是使用js动态生成的，要找到数据的来源，可以使用chrome工具中的Network，查看网页在加载过程中加载了哪些数据，选择查看XHR类型，可以找到如下图所示的数据：

![Network-XHR]({{ site.url }}/assets/pic9172215.png)

分析数据，可以看出，在json数据中，["data"]["vlist"]中的各项list均为稿件信息，其中每个list中的["pic"]即为视频封面的地址。

![数据结构分析]({{ site.url }}/assets/pic9172218.png)
