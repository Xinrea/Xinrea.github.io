...
为了解决这一问题，我们需要集群本身具有 C<sub>0</sub> 节点的切换机制，不能依赖外部服务。
...
提到分布式共识算法就得从 [Paxos 算法](https://zhuanlan.zhihu.com/p/31780743) 说起。Paxos 算法模拟了一个小岛上通过决议的流程，一个值的确定需要多数人发起提案（Prepare 阶段）后，得到多数人的同意（Accept 阶段）。其中每个人都要维护一个Proposal ID，已确保只处理自身视角里最新的 Proposal，Proposal ID 将会在 Prepare 和 Accept 阶段中更新。
